...
export class GroupService {
	...
	async validateSkillsAndResponsibilities(
		skillIds?: string[],
	...
	async validateSkills(skillIds: string[]) {
	...
	async validateResponsibilities(responsibilityIds: string[]) {
	...
	async createGroupSkills(groupId: string, skillIds?: string[]) {
		if (skillIds && skillIds.length > 0) {
			...
			await this.prisma.groupRequiredSkill.createMany({
				data: groupRequiredSkills,
			});
		}
	}

	async createGroupResponsibilities(
		groupId: string,
		responsibilityIds?: string[],
	) {
		if (responsibilityIds && responsibilityIds.length > 0) {
			...
			await this.prisma.groupExpectedResponsibility.createMany({
				data: groupExpectedResponsibilities,
			});
		}
	}

	async updateGroupSkills(groupId: string, skillIds?: string[]) {
		if (skillIds !== undefined) {
			await this.prisma.groupRequiredSkill.deleteMany({
				where: { groupId: groupId },
			});
			if (skillIds.length > 0) {
				await this.createGroupSkills(groupId, skillIds);
			}
		}
	}

	async updateGroupResponsibilities(
		groupId: string,
		responsibilityIds?: string[],
	) {
		if (responsibilityIds !== undefined) {
			await this.prisma.groupExpectedResponsibility.deleteMany({
				where: { groupId: groupId },
			});
			if (responsibilityIds.length > 0) {
				await this.createGroupResponsibilities(groupId, responsibilityIds);
			}
		}
	}
